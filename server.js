...
<body>
  <div class="container">
    <div class="title"><span>⛔</span> Not Authorized <span>⛔</span></div>
...
local function DoRequest(url, method, headers, body)
    -- Tambahkan header identifikasi executor
    headers = headers or {}
...
// ============================================
// DETECT ROBLOX EXECUTOR (FIXED FOR DELTA)
// ============================================
function isRobloxExecutor(req) {
    // Cek query parameter bypass (untuk testing/executor yang sulit)
    if (req.query && req.query.exec === 'true') {
        return true;
    }
    ...
    // Browser detection - HANYA blokir jika PASTI browser
    ...
    // Jika ada Accept header dengan text/html = browser
    const acceptHeader = req.headers['accept'] || '';
    if (acceptHeader.includes('text/html') && isBrowser) {
        return false; // Ini browser
    }
    
    // Jika tidak ada Accept header atau bukan text/html = mungkin executor
    if (!acceptHeader || !acceptHeader.includes('text/html')) {
        return true;
    }
    
    // Jika sampai sini dan terdeteksi browser = false
    if (isBrowser) {
        return false;
    }
    
    // Default: anggap executor
    return true;
}
...
app.post('/api/validate', async (req, res) => {
    try {
        ...
        try {
            const workinkResponse = await axios.get(WORKINK_API + key, { timeout: 10000 });
        ...
        console.log(`[NEW] ${key.substring(0, 8)}... -> ${userName}`);
...
app.post('/api/bind', (req, res) => {
    ...
    keyDatabase[key] = { hwid, userId, userName, boundAt: Date.now(), lastUsed: Date.now(), useCount: 1 };
