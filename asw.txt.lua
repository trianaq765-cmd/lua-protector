--[[
    🛡️ SHIELD SECURITY SYSTEM v3.1 (Passive Mode)
...
if getgenv()._SHIELD_SESSION_KEY then getgenv()._SHIELD_SESSION_KEY=nil getgenv()._SHIELD_TIMESTAMP=nil end
local function _SECURITY_KICK(reason)pcall(function()game:GetService("Players").LocalPlayer:Kick("⛔ "..tostring(reason))end)while true do task.wait(9e9)end end
if not _SEC_TOKEN then _SECURITY_KICK("Security Violation: Script executed manually (No Session Token).")return end
local SPY_GUI_NAMES={"simplespy","simple_spy","remotespy","remote_spy","httpspy","http_spy","dexexplorer","dex","darkdex","infiniteyield","hydroxide","scriptdumper","unluac","jobid","rejoin","explorer"}
local function CheckGUI(child)if not child then return end if child:IsA("ScreenGui")or child:IsA("BillboardGui")then local name=string.lower(child.Name)for _,spy in ipairs(SPY_GUI_NAMES)do if name:find(spy)then _SECURITY_KICK("Spy Tool Detected: "..child.Name)return true end end task.spawn(function()local suspicious_score=0 for _,desc in ipairs(child:GetDescendants())do if desc:IsA("TextLabel")or desc:IsA("TextButton")then local text=string.lower(desc.Text)if text:find("remote")and text:find("spy")then suspicious_score=suspicious_score+1 end if text:find("function")and text:find("hook")then suspicious_score=suspicious_score+1 end if text:find("saveinstance")then suspicious_score=suspicious_score+1 end end end if suspicious_score>=2 then _SECURITY_KICK("Spy Tool Detected (Content Scan): "..child.Name)end end)end end
local CG=game:GetService("CoreGui")CG.ChildAdded:Connect(CheckGUI)for _,c in ipairs(CG:GetChildren())do CheckGUI(c)end

--[[ ═══════════════════════════════════════════════════════
     CORE SCRIPT - KEY SYSTEM WITH JUNKIE INTEGRATION
═══════════════════════════════════════════════════════ ]]
...
-- CONFIG (Junkie Integration)
local CFG={
    -- Junkie Dashboard Settings
    junkie_service = "ToingDc",        -- Name of Service from Junkie dashboard
    junkie_identifier = "4005",   -- The id of your provider name
    junkie_provider = "hallo",       -- Junkie provider name
    
    -- Script Settings
    HL = "https://raw.githubusercontent.com/trianaq765-cmd/VD-key-sistem/refs/heads/main/loader%20new%20(5).lua.txt", -- Script Loader setelah Key valid
    MA = 3,                           -- Max attempts before kick
    SF = "ToingDc",                   -- Save folder name
    SFL = "KeyData.json",             -- Save file name for key storage
}

local Junkie=loadstring(game:HttpGet("https://jnkie.com/sdk/library.lua"))()
Junkie.service = "ToingDc"
Junkie.identifier = "11515"
Junkie.provider = "hallo"

local errorMessages={KEY_INVALID="Key tidak ditemukan",KEY_EXPIRED="Key sudah expired",HWID_BANNED="Hardware banned",KEY_INVALIDATED="Key dinonaktifkan",ALREADY_USED="Key sekali pakai sudah terpakai",HWID_MISMATCH="HWID limit tercapai",SERVICE_NOT_FOUND="Service tidak ditemukan",SERVICE_MISMATCH="Key untuk service lain",PREMIUM_REQUIRED="Butuh key premium",ERROR="Network error, coba lagi",RATE_LIMITTED="Rate limited, tunggu 5 menit"}
local FA=0
...
local function GEN()local n="Unknown"pcall(function()if identifyexecutor then local a=identifyexecutor()if a then n=tostring(a)end end end)return n end

if game.PlaceId~=GID then SN("⛔ Wrong Game","Script ini hanya untuk Violence District!",3)end
...
local function VK(k)if not k or type(k)~="string"or #k<5 then return{valid=false,serverReachable=true,message="Key terlalu pendek"}end k=k:gsub("%s+","")local s,rs=pcall(function()return Junkie.check_key(k)end)if not s then return{valid=false,serverReachable=false,message="Tidak bisa konek ke server"}end if rs.valid then return{valid=true,serverReachable=true}else local err=rs.error or"Unknown error"local msg=errorMessages[err]or err if err=="HWID_BANNED"then task.spawn(function()task.wait(2)LP:Kick("Hardware banned from service")end)end return{valid=false,serverReachable=true,message=msg}end end

local function CSK()local sd=LK()if sd and sd.key then local rs=VK(sd.key)if rs.valid==true then return true elseif rs.serverReachable==true then DK()SN("⚠️ Key Expired","Key tersimpan tidak valid, masukkan key baru",4)return false else SN("⚠️ Offline Mode","Server tidak bisa dihubungi",3)return true end end return false end
...
GK.MouseButton1Click:Connect(function()SS.Text="Generating link..."SS.TextColor3=Color3.fromRGB(255,200,100)local s,link,err=pcall(function()return Junkie.get_key_link()end)if s and link then if setclipboard then setclipboard(link)SS.Text="✅ Link copied!"SS.TextColor3=Color3.fromRGB(100,180,255)SN("ToingDc","Key link copied to clipboard!",4)else SS.Text="Check console for link"print("Get Key Link:",link)end else local errMsg=err or"Failed to generate"if errMsg=="RATE_LIMITTED"or(type(err)=="string"and err:find("RATE"))then SS.Text="⚠️ Rate limited, tunggu 5 menit"SS.TextColor3=Color3.fromRGB(255,150,100)else SS.Text="❌ "..tostring(errMsg)SS.TextColor3=Color3.fromRGB(255,100,100)end end end)

local IV=false
SB.MouseButton1Click:Connect(function()if IV then return end local k=KI.Text:gsub("%s+","")if k==""then SS.Text="Key tidak boleh kosong!"SS.TextColor3=Color3.fromRGB(255,100,100)SHI()return end IV=true SS.Text="Validating..."SS.TextColor3=Color3.fromRGB(255,200,100)task.wait(0.5)local rs=VK(k)IV=false if rs.valid==true then SS.Text="✅ Key valid! Loading..."SS.TextColor3=Color3.fromRGB(100,255,140)SK(k)task.wait(1)SGui:Destroy()pcall(function()loadstring(game:HttpGet(CFG.HL))()end)elseif rs.serverReachable==false then SS.Text="⚠️ "..rs.message SS.TextColor3=Color3.fromRGB(255,180,100)SHI()else FA=FA+1 UUI()SHI()local rm=GR()if rm<=0 then SS.Text="❌ Key salah 3x!"task.wait(1.5)KPK("masukin key kocak")else SS.Text="❌ "..rs.message.." | Sisa: "..rm SS.TextColor3=Color3.fromRGB(255,100,100)end end end)
UUI()